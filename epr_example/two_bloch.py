...
class TwoQubitColoredBlochSpheres(ThreeDScene):
    def get_bloch_sphere(self, sphere_color=BLUE):
        # Bloch Sphere
        # Bloch Sphere (color fill + stroke)
        ...
        # Axes: keep consistent across both
        ...
        # State vector - match color to sphere
        state_vector_arrow = Arrow3D(start=[0, 0, -1.0], end=[0, 0, 1.5], color=RED)
    ...
    def construct(self):
        ...
        red_qubit = self.get_bloch_sphere(sphere_color=YELLOW).shift(RIGHT * 3 + DOWN * 0.5 + IN * 1.5)
        blue_qubit = self.get_bloch_sphere(sphere_color=YELLOW).shift(LEFT * 3 + UP * 0.2 + OUT * 0.5)

        self.add(red_qubit, blue_qubit)
        self.wait(3)
